...
from Algorithm.QVPO.helpers import cosine_beta_schedule, linear_beta_schedule, vp_beta_schedule, extract, Losses

from Algorithm.QVPO.model import Model


class Diffusion(nn.Module):
    def __init__(self, state_dim, action_dim, noise_ratio, beta_schedule="vp", n_timesteps=1000, loss_type="l2", clip_denoised=True, predict_epsilon=True, behavior_sample=16, eval_sample=512, deterministic=False):
    ...
    def sample_n(self, state, eval=False, times=32, chosen=1, q_func=None, origin=None):
        ...
        if chosen == 1:
            ...
            action_idx = q_idx.repeat(1, 1, self.action_dim)
            return (
                old_state.repeat(chosen, 1).view(chosen, raw_batch_size, -1).transpose(0, 1).contiguous().view(raw_batch_size * chosen, -1),
                action.gather(dim=1, index=action_idx).view(raw_batch_size * chosen, -1),
                (q.view(raw_batch_size * chosen, 1), v),
                (mean, std),
            )
