...
class Diffusion(nn.Module):
    ...
    def sample(self, state, eval=False):
        self.noise_ratio = 0.5 if eval else self.max_noise_ratio
...
class DiPo_MLP_Agent(object):
    def __init__(
        ...
        self.actor = Diffusion(state_dim=state_dim, action_dim=action_dim, noise_ratio=args.noise_ratio, beta_schedule=args.beta_schedule, n_timesteps=args.diffusion_steps).to(device)
        ...
        self.action_gradient_steps = args.action_update_epochs
    ...
    def select_action(self, state, steps, eval):

        if steps < self.threshod and eval == False:
            action = torch.rand(self.action_dim).unsqueeze(dim=0).to(self.device)

        else:
            action = self.actor(state, eval)
            action = action * self.action_scale + self.action_bias
        # assert action.size() == (1, self.action_dim)
        return action

    def action_gradient(self, batch_size, log_writer):
        states, best_actions, idxs = self.diffusion_memory.sample(batch_size, squeeze=True)
        ...
        if self.update_num % 10 == 0:
            log_writer.add_scalar("algorithm/Action_Grad_Norm", actions_grad_norms.max().item(), self.update_num)

        best_actions = best_actions.detach()

        self.diffusion_memory.replace(idxs, best_actions, squeeze=True)

        return states, best_actions

    def train(self, epochs_num, batch_size=256, log_writer=None):
        for _ in range(epochs_num):
            ...
            # cancat_q1q2 = torch.concat((target_q1, target_q2), dim=1)  # [bs,1],[bs,1]-->[bs,2]然后对dim=0使用torch.min
            # target_q, _ = torch.min(cancat_q1q2, dim=1, keepdim=True)

            target_q = (rewards.unsqueeze(dim=1) + self.gamma * target_q).detach()
            ...
            if self.cri_grad_norm > 0:
                ...
                if self.update_num % 10 == 0:
                    log_writer.add_scalar("algorithm/Critic_Grad_Norm", critic_grad_norms.max().item(), self.update_num)
            ...
            if self.ac_grad_norm > 0:
                ...
                if self.update_num % 10 == 0:
                    log_writer.add_scalar("algorithm/Actor_Grad_Norm", actor_grad_norms.max().item(), self.update_num)
            ...
            if self.update_num % self.update_actor_target_every == 0:
        ...
        # if self.update_num % 10 == 0:
        #     self.writer.add_scalar("algorithm/value_loss", critic_loss.item(), global_step=self.update_num)
        #     self.writer.add_scalar("algorithm/policy_loss", actor_loss.mean().item(), global_step=self.update_num)

        #     self.writer.add_scalar("algorithm/ratio", ratios.mean().item(), global_step=self.update_num)
