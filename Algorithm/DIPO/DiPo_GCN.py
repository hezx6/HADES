...
class GCN_Predict_Model(nn.Module):  # 类似于Actor的核心网络，用来预测噪声或者函数值，预测网络的推理是Actor forward的一个步骤
...
class GCN_Critic(nn.Module):  # 这是算法训练时需要用到的Critic函数，用来预测Q值
...
class GCN_Diffusion(nn.Module):  # 这是算法的核心实现(整个实现相当于一个Actor，model infer只是Actor的一部分)
    def __init__(
        self,
        net_node_feat_dim,
        dag_node_feat_dim,
    ...
    def q_sample(self, x_start, t, noise=None):  # 前向过程，对原始数据加高斯噪声
        if noise is None:
    ...
    def p_losses(self, x_start, state, t, weights, batch_size):
        noise = torch.randn_like(x_start)  # 高斯分布
        ...
        if self.predict_epsilon:
            loss = self.loss_fn(x_recon, noise, weights)  # 计算重建后的值与噪声的loss，让self.model拟合噪声
        else:
            loss = self.loss_fn(x_recon, x_start, weights)  # 计算重建后的值与真实值的loss，让self.model学习从噪声映射真实值
...
class DiPo_GCN(object):  # 这是算法运行过程的封装
